...
| **Provisioning sidecar** | `~/.config/metafactory/agents/<agent-id>.provision.json` | every agent | chmod 600. The **canonical** provisioning record and idempotency anchor (arc#281 option (a)). A small JSON: `{ agent_id, did, provisioned, provisioned_at, nkey_seed_path, nkey_pub, state_scaffolded, instance_dir? }`. Written for stateful **and** stateless agents. |
