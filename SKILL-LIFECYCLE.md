...
| Three-tier persistence | Secrets, config, runtime state | `~/.config/metafactory/`, `pai-personal-data/` |
...
Tier 1: Secrets (API tokens)
  ~/.config/metafactory/secrets/{skill-name}.env
...
Tier 3: Runtime state (cache, rate limits, last-run)
  ~/.config/metafactory/skills/{skill-name}/
...
~/.claude/secrets/jira-work.env
  → ~/.config/metafactory/secrets/jira-work.env               (Tier 1)
```
...
# Copy secrets to ~/.config/metafactory/secrets/jira-work.env
...
# 2. FETCH: git clone to ~/.config/metafactory/pkg/staging/extract-wisdom/
...
# 8. RECORD: write to ~/.config/metafactory/packages.db
```
...
| Secrets stranded in old version | Flat files in `~/.claude/secrets/` | Tier 1: `~/.config/metafactory/secrets/` |
...
#    .env → ~/.config/metafactory/.env
#    CLAUDE.md → pai-personal-data/CLAUDE.md
#    MEMORY → ~/.config/metafactory/MEMORY
#    profiles → pai-personal-data/profiles
#    secrets → ~/.config/metafactory/secrets
#    PAI/USER → ~/.config/metafactory/CORE_USER
#
...
| Three-tier persistence | ✅ Designed + implemented | `~/.config/metafactory/`, `pai-personal-data/` | Working for custom skills |
...
| **packages.db** | ✅ Built | `~/.config/metafactory/packages.db` | SQLite via bun:sqlite, WAL mode |
