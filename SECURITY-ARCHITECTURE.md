...
```yaml
# ~/.config/metafactory/security/anomaly-rules.yaml
...
rules:
  ...
  - name: "data-staging"
    ...
    trigger:
      sequence:
        - event: {tool: "Read", path_matches: "~/.config/metafactory/secrets/*"}
...
# → Installs gitleaks rules to ~/.config/metafactory/security/
...
- Install to a protected path (`~/.config/metafactory/system-skills/`) separate from user skills
